...
apacheGo() {
  ...
  sudo cp /vagrant/data/apache/lamp7.vh.conf /etc/apache2/sites-available/lamp7.vh.conf && \
  sudo ln -s /etc/apache2/sites-available/lamp7.vh.conf /etc/apache2/sites-enabled/lamp7.vh && \
...
phpGo() {
  ...
  sudo ln -s /etc/php/7.3/mods-available/lamp.ini /etc/php/7.3/fpm/conf.d/100-lamp.ini && \
